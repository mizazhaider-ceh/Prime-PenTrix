"""
Prime PenTrix - Query Expander
==============================
Expands user queries with cybersecurity domain synonyms/abbreviations
and subject-specific context terms for improved BM25 + semantic retrieval.
...
class QueryExpander:
    ...
    # ── Cybersecurity Domain Expansions ──────────────────────
    ...
    EXPANSIONS = {
        ...
        "ddos": ["distributed denial of service", "ddos", "dos"],
    ...
    # ── Subject-Specific Context Terms ──────────────────────
    ...
    SUBJECT_CONTEXT = {
        ...
        "aisec": ["adversarial", "model security", "AI attack", "LLM", "neural network"],
    }
    ...
    def expand(cls, query: str, subject_id: Optional[str] = None) -> str:
        ...
        Args:
            ...
            subject_id: Optional subject for context-specific expansion

        Returns:
            Expanded query string
        """
        lower_query = query.lower().strip()
        expansion_terms: set = set()

        # Add matching domain expansions
        for key, expansions in cls.EXPANSIONS.items():
            if key in lower_query:
                expansion_terms.update(expansions)

        # Add subject context terms (up to 3)
        ...
        if expansion_terms:
            ...
            existing_words = set(lower_query.split())
            new_terms = expansion_terms - existing_words
            if new_terms:
                extra = " ".join(new_terms)
                expanded = f"{query} {extra}"
                logger.debug(
                    f"Query expanded: '{query}' → '{expanded[:120]}...'"
                )
                return expanded
